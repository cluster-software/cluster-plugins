...
2. Call `enrich_contact_info` with `scope="first_5"` and `dry_run=false`; request
   only the fields the user needs, normally `work_email` before `phone`. Supply
   `input_column_id` only when Ethos cannot infer the profile column.
...
4. Before the broader run, call `enrich_contact_info` with the returned
   `column_id`, `scope="empty"`, and `dry_run=true`. At or below 100 estimated
   credits, repeat with `scope="empty"` and `dry_run=false` without another
   confirmation. Above 100, show the estimate and obtain approval, then repeat
   with `scope="empty"`, `dry_run=false`, and that estimate as
   `acknowledged_credits`.
5. Use `force_refresh` only when the user explicitly needs to bypass cached
   contact data. Wait on the new run and report requested fields, selected rows,
   credits, terminal status, and failures.
