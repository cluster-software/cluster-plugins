...
2. Reuse a complete engager table when supplied. Otherwise call `source_signal`
   ...
   and `max_comments`. Use one idempotency key. Handle a >100-credit quote, then wait with
   `get_job_status(wait_seconds=120)`. A successful no-match result ends the run.
3. Call `enrich_table` on a small sample with one structured qualification
   column: `qualified:boolean`, `reasoning:text`, and `evidence:text`. Require
   company fit, persona fit, current employment, verified evidence, and explicit
   disqualifiers; unclear cases default false. Wait with `get_job_status`, QA the
   sample, then run the broader selection.
...
6. Call `build_campaign_draft` with the qualified `leads` selection, mappings,
   sequence, settings, and any campaign AI variables. It creates/stages the
   list and draft but never launches. Verify it with `analyze_campaigns`.
