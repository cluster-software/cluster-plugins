---
name: setup
description: Set up or repair Ethos by verifying the marketplace plugin and hosted MCP connection, plus the required CLI on Claude Code and Codex or the optional CLI in Claude Desktop/Cowork.
...
- Claude Code in a terminal (`/plugin` slash commands available to the user),
- Claude Code run by the Claude desktop app
  (`CLAUDE_CODE_ENTRYPOINT=claude-desktop` with the shell on the user's real
  operating system; `/plugin` dialog commands unavailable),
- a Claude Desktop/Cowork conversation (the shell is a Linux sandbox VM behind
  an egress proxy), or
- Codex.

Use the product that is running this skill as the source of truth. Do not infer
Claude Desktop/Cowork from a failed network request alone, and do not infer a
host shell from the presence of a `claude` command: the Desktop/Cowork sandbox
VM ships its own `claude` CLI whose configuration is sandbox-local, never read
by the app, and discarded after the session. Shell `claude plugin` output is
authoritative in both Claude Code flavors and meaningless in Claude
Desktop/Cowork; when the host is ambiguous, do not trust it.

The completion requirements depend on that host:

- Claude Code (terminal or desktop-run) and Codex require a working,
  authenticated `ethos-cli`, the installed plugin, and the plugin's
  authenticated hosted MCP server.
...
major version is below 20, tell the user exactly what is missing. In Claude
Code or Codex, stop the CLI portion and ask the user to install a current
...
non-blocking only in Claude Desktop/Cowork; in Claude Code and Codex they
...
The shell runs on the user's real machine here, so `claude plugin list --json`
is authoritative. Inspect the plugin state:
...
the install path depends on the flavor:

- **Terminal:** show the user the exact slash commands from
  `GETTING_STARTED.md` (`/plugin marketplace add
  cluster-software/cluster-plugins`, then
  `/plugin install ethos@cluster-plugins`), wait for them to confirm they ran
  both, and inspect again.
- **Desktop-run:** the `/plugin` dialogs do not exist; run
  `claude plugin marketplace add cluster-software/cluster-plugins` and
  `claude plugin install ethos@cluster-plugins` yourself and let the user
  approve them. If the permission mode declines the commands, hand the user
  those same two commands to run in any regular terminal.
...
- The entry is listed but this session shows no Ethos MCP server in any state:
  the session is stale. In the terminal, ask the user to run
  `/reload-plugins`, then invoke `/ethos:setup` again in this conversation. In
  the desktop-run flavor, MCP servers load only at session start: ask the user
  to start a new conversation and invoke `/ethos:setup` there. After the
  reload the server may still need authentication; finish with step 4.
- The entry is missing even though the plugin is installed and enabled: the
  ...
  problem, not a stale session. In the terminal, have the user run
  `/plugin marketplace update cluster-plugins` then
  `/plugin update ethos@cluster-plugins`; in the desktop-run flavor, run
  `claude plugin marketplace update cluster-plugins` and
  `claude plugin update ethos@cluster-plugins` yourself. Re-check afterwards;
  if the entry is still missing, report the plugin version and this exact
  symptom as a plugin configuration bug and stop.
...
For Claude Code and Codex, report the organization name and ID along with
...
| Node.js or npm is missing | In Claude Code or Codex, leave the plugin installed and guide the user to install Node.js 20+ before resuming. In Claude Desktop/Cowork, report the optional CLI as unavailable and continue to MCP. |
...
| `/plugin` reports it isn't available in this environment | The session is Claude Code run by the Claude desktop app. Use the shell install path for that flavor: `claude plugin marketplace add` / `claude plugin install`, then a new conversation. |
| MCP tools are absent in Claude | In Claude Code run `claude mcp list` first: if `plugin:ethos:ethos` is listed and the session shows the server awaiting authentication, complete the MCP OAuth in step 4 — no reload; if it is listed but the session shows no Ethos server, run `/reload-plugins` (or start a new conversation in the desktop-run flavor); if it is missing, update the marketplace and plugin — reload cannot fix an entry Claude skipped at parse time. In Claude Desktop/Cowork fully quit and reopen the app. Then retry the skill. |
