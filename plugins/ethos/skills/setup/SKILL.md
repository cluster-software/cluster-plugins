...
After the plugin check, if the MCP tool is unavailable: in the terminal ask
the user to run `/reload-plugins`, then invoke `/ethos:setup` again in this
conversation; in the desktop-run flavor ask the user to start a new
conversation and invoke `/ethos:setup` there, since `/reload-plugins` is
unavailable. If the tool is already available, continue directly to the
read-only call in step 4.
...
| MCP tools are absent in Claude | In terminal Claude Code run `/reload-plugins`; in desktop-run Claude Code start a new conversation; in Claude Desktop/Cowork fully quit and reopen the app. Then retry the skill. |
