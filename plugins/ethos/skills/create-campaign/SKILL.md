...
3. Resolve the connected LinkedIn sender account. Launch requires `connected_account_id` on the campaign, and MCP can only set it at creation. Call `list_campaigns` and reuse the `connected_account_id` from a past campaign, or ask the user. If neither works, create the campaign without it and have the user pick the sender on the campaign page before launch.
...
3. **Create the campaign.** Call `create_campaign_with_sequence` with `name`, `messages` (each `{name?, body_template, wait_amount, wait_unit}`), `list_id`, `connected_account_id` (omit if unresolved; the user then picks the sender on the campaign page before launch), and `first_step` (`invitation` opens with a connection request), plus optional `copy_play` and `ai_variables` from the chosen draft.
