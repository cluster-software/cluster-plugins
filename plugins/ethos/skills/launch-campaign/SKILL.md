...
1. If the user already has a campaign, call `add_leads_to_campaign` with a table source or contact IDs. Drafts import without enrolling; running campaigns enroll immediately.
...
3. To launch a ready draft, call `launch_campaign` - destructive: it starts real LinkedIn sends, so confirm with the user first. The campaign must already have a lead list and a connected LinkedIn sender account (set at creation or on the campaign page).
